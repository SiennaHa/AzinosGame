...
public class Dialogue : MonoBehaviour
{
    ...
    public void StartAzinosDialogue()
    {
        waiting = true;
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (waitForNext && hit.collider == colliderEyes || hit.collider == colliderHorns || hit.collider == colliderBoobs || hit.collider == colliderSnake || hit.collider == colliderHair)
            {
                waitForNext = false;
                //indexStart++;
